...
func (node *RadixNode) Insert(entryID string, data []string) {
	idx := findMismatchIndex(node.label, entryID)
	...
	case len(node.label):
		if idx == len(entryID) {
			...
			child := node.children[charToIndex(entryID[idx])]
			if child == nil {
				node.children[charToIndex(entryID[idx])] = &RadixNode{
					label: entryID[idx:],
					data:  data,
				}
			} else {
				child.Insert(entryID[idx:], data)
			}
		}
...
func (node *RadixNode) Search(entryID string) ([]string, bool) {
	idx := findMismatchIndex(node.label, entryID)
	...
	childIdx := charToIndex(entryID[idx])
	if childIdx == -1 {
		return nil, false
	}
	child := node.children[childIdx]
	if child != nil {
		return child.Search(entryID[idx:])
	}
